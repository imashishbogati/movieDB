...
class MovieListView: UIView {
    ...
    // MARK: - Life Cycle
    required init(viewModel: MovieViewModel, listType: MovieListType) {
    ...
    private func loadData() {
        headingLabel.text = viewModel?.listTitle(listType: self.listType)
        viewModel?.getMovies(listType: self.listType,
                             page: "1", completion: { [weak self] response in
